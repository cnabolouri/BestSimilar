import type { TitleNewsItem } from "@/types/title";
    ...
    <section className="rounded-[2rem] border border-border bg-card p-6 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-muted-foreground">
            Coverage, updates, and related activity around this title.
          </p>
        </div>
      ...
      {newsItems.length === 0 ? (
        <div className="mt-5 rounded-2xl border border-dashed border-border px-4 py-5 text-sm text-muted-foreground">
          No recent news has been added for this title yet.
        </div>
      ) : (
