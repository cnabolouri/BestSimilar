...
export function BrowseTitleCard({ title }: { title: BrowseTitle }) {
  ...
  return (
    <Link
      href={href}
      className="simcine-card group rounded-3xl border border-border bg-card p-[var(--simcine-card-padding)] transition hover:border-accent/60 hover:shadow-sm"
    >
      ...
      <div className="mt-3">
        <h3 className="line-clamp-2 text-[length:var(--simcine-card-title-size)] font-semibold group-hover:text-accent">
          {title.name}
        </h3>
        ...
        {title.genres.length > 0 ? (
          <p className="mt-1 line-clamp-1 text-[length:var(--simcine-card-meta-size)] text-muted-foreground">
            {title.genres.slice(0, 3).map((genre) => genre.name).join(" • ")}
          </p>
        ) : null}

        <p className="mt-2 text-[length:var(--simcine-card-meta-size)] text-muted-foreground">
