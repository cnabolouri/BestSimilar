  ...
  return (
    <div>
      ...
      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
        {credits.map((credit) => {
          const href = credit.title_media_type === "movie" ? `/movie/${credit.title_slug}` : `/tv/${credit.title_slug}`;

          return (
            <Link
              key={credit.id}
              href={href}
              className="rounded-2xl border border-border bg-card p-4 transition hover:border-accent"
            >
